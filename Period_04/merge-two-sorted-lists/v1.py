...
class Solution:
    def mergeTwoLists(self, list1: Optional[ListNode], list2: Optional[ListNode]) -> Optional[ListNode]:
        # Создаем фиктивный узел (dummy node) для упрощения логики
        dummy = ListNode(0)
        current = dummy
        
        # Идем по обоим спискам, пока не дойдем до конца хотя бы одного
        ...
        # Присоединяем оставшиеся элементы (если они есть)
        if list1:
            current.next = list1
        elif list2:
            current.next = list2
        
        # Возвращаем следующий узел после фиктивного (первый реальный элемент)
        return dummy.next
